...
if __name__ == '__main__':
    ...
    recordObj = data.DataRecord(fileName)
    while not done:
        op = menu.GetMenuInput()
        if op == 1:
            recordObj.InputUserData()
        elif op == 2:
            recordObj.InputPassword()
        elif op == 3:
            recordObj.VerifyUserNameAndWriteToFile()
        elif op == 4:
            recordObj.DisplayRecords()
        elif op == 5:
            recordObj.VerifyLogin()
        elif op == 6:
            recordObj.SortByLastName()
        elif op == 7:
            done = True
        else:
            pass
